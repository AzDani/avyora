...
const avant = await p.evaluate(() => { const lv = L(), cl = lv.walls.find((w) => w.type === "cloison" && !w.st); sel = { kind: "wall", id: cl.id }; render();
  return { cl: cl.id, nW: lv.walls.length, nT: chantierTasks().length, px: chantierPrix().total, bouton: [...document.querySelectorAll("#pbody .danger, #pbody .lsuppr")].map((x) => x.textContent.trim()), decision: [...document.querySelectorAll("#pbody .segetat button")].map((x) => x.textContent), hint: document.getElementById("hint").textContent }; });
/* D50 (cj-design04) : « À démolir » est dans la liste Décision, plus un second bouton rouge (le rouge = à créer) */
t["Travaux · la fiche d'un mur existant : « À démolir » dans la Décision, aucun bouton rouge ni « Supprimer du plan »"] = avant.decision.some((x) => /À démolir/.test(x)) && avant.bouton.length === 0;
...
Object.assign(t, await p.evaluate(() => { const r = {}, lv = L();
  ...
  r["mur créé en Travaux : sa fiche propose « Supprimer du plan »"] = [...document.querySelectorAll("#pbody .lsuppr")].some((x) => x.textContent.trim() === "Supprimer du plan");
  deleteSel();
